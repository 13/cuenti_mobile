...
void main() {
  ...
  test('getAll parses list of tags', () async {
    when(() => dio.get<List<dynamic>>('/tags')).thenAnswer(
      (_) async => ok([
        {'id': 1, 'name': 'Work'},
        {'id': 2, 'name': 'Personal'},
      ]),
  ...
  test('getAll passes search query param when non-null', () async {
    ...
    verify(
      () =>
          dio.get<List<dynamic>>('/tags', queryParameters: {'search': 'work'}),
    ).called(1);
  });
  ...
  test('save posts new tag when id is null', () async {
    ...
    when(
      () => dio.post<Map<String, dynamic>>('/tags', data: any(named: 'data')),
  ...
  test('save puts existing tag when id is set', () async {
    ...
    when(
      () => dio.put<Map<String, dynamic>>('/tags/7', data: any(named: 'data')),
