...
void main() {
  ...
  test('save posts new scheduled transaction when id is null', () async {
    ...
    expect(
      captured['nextOccurrence'],
      isA<String>().having(
        (s) => s.startsWith('2026-07-19T00:00:00'),
        'ISO date',
        true,
      ),
    );
  ...
  test('delete calls DELETE /scheduled-transactions/{id}', () async {
    when(
      () => dio.delete<void>('/scheduled-transactions/1'),
  ...
  test(
    'post calls POST /scheduled-transactions/{id}/post and returns result',
    () async {
      when(
        () => dio.post<Map<String, dynamic>>('/scheduled-transactions/1/post'),
  ...
  test(
    'skip calls POST /scheduled-transactions/{id}/skip and returns result',
    () async {
      when(
        () => dio.post<Map<String, dynamic>>('/scheduled-transactions/1/skip'),
      ).thenAnswer((_) async => ok({'id': 1, 'type': 'EXPENSE'}));
